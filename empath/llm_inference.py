...
from empath.constrained_logits_processor import MetadataConstrainedLogitsProcessor
from empath.constants import DEFAULT_LM_INSTRUCTION, DEFAULT_LM_UNDERSTAND_INSTRUCTION, DEFAULT_LM_INSPIRED_INSTRUCTION, DEFAULT_LM_REWRITE_INSTRUCTION, DURATION_MIN, DURATION_MAX
from empath.gpu_config import get_lm_gpu_memory_ratio, get_gpu_memory_gb, get_lm_model_size, get_global_gpu_config
...
class LLMHandler:
    ...
    def __init__(self, persistent_storage_path: Optional[str] = None):
        ...
        self.disable_tqdm = os.environ.get("EMPATH_DISABLE_TQDM", "").lower() in ("1", "true", "yes") or not (hasattr(sys.stderr, 'isatty') and sys.stderr.isatty())
    ...
    def get_available_5hz_lm_models(self) -> List[str]:
        """Scan and return all model directory names starting with 'empath-5Hz-lm-'"""
        ...
        if os.path.exists(checkpoint_dir):
            for item in os.listdir(checkpoint_dir):
                ...
                if os.path.isdir(item_path) and item.startswith("empath-5Hz-lm-"):
                    models.append(item)
    ...
    def get_gpu_memory_utilization(self, model_path: str = None, minimal_gpu: float = 8, min_ratio: float = 0.2, max_ratio: float = 0.9) -> Tuple[float, bool]:
        ...
        Args:
            model_path: LM model path (e.g., "empath-5Hz-lm-0.6B"). Used to determine target memory.
    ...
    def initialize(
        ...
        try:
            ...
            if lm_model_path is None:
                lm_model_path = "empath-5Hz-lm-1.7B"
    ...
    def _initialize_5hz_lm_vllm(self, model_path: str, enforce_eager: bool = False) -> str:
        ...
        try:
            ...
            self.llm_initialized = False
            logger.error("nano-vllm is not installed. Please install it using 'cd empath/third_parts/nano-vllm && pip install .")
            return "❌ nano-vllm is not installed. Please install it using 'cd empath/third_parts/nano-vllm && pip install ."
    ...
    def _is_mlx_available() -> bool:
        ...
        """
        try:
            from empath.models.mlx import mlx_available
    ...
    def _load_mlx_model(self, model_path: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            try:
                ...
                # The Empath 5Hz LM checkpoints store safetensors keys without
    ...
    def _run_mlx_batch_native(
        ...
        *slows down* throughput for 1.7B-class models. Instead, we:
        ...
        from empath.constrained_logits_processor import FSMState
