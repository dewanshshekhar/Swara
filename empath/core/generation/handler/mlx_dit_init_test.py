...
MLX_DIT_INIT_MODULE = _load_handler_module(
    "mlx_dit_init.py",
    "empath.core.generation.handler.mlx_dit_init",
)
...
class MlxDitInitMixinTests(unittest.TestCase):
    ...
    def test_init_mlx_dit_success_sets_decoder(self):
        ...
        fake_dit_model = types.ModuleType("empath.models.mlx.dit_model")
        fake_dit_model.MLXDiTDecoder = type(
            "FakeDecoder",
            (),
            {"from_config": classmethod(lambda _cls, _cfg: object())},
        )
        fake_dit_convert = types.ModuleType("empath.models.mlx.dit_convert")
        fake_dit_convert.convert_and_load = Mock()
        with patch.dict(
            sys.modules,
            {
                "empath.models.mlx": fake_mlx,
                "empath.models.mlx.dit_model": fake_dit_model,
                "empath.models.mlx.dit_convert": fake_dit_convert,
            },
        ):
