...
def _load_init_service_module():
    ...
    package_paths = {
        "empath": repo_root / "empath",
        "empath.core": repo_root / "empath" / "core",
        "empath.core.generation": repo_root / "empath" / "core" / "generation",
        "empath.core.generation.handler": repo_root / "empath" / "core" / "generation" / "handler",
    }
    ...
    spec = importlib.util.spec_from_file_location(
        "empath.core.generation.handler.init_service",
        module_path,
    )
...
INIT_SERVICE_DOWNLOADS_MODULE = importlib.import_module("empath.core.generation.handler.init_service_downloads")
INIT_SERVICE_MEMORY_BASIC_MODULE = importlib.import_module("empath.core.generation.handler.init_service_memory_basic")
...
class InitServiceMixinTests(unittest.TestCase):
    ...
    def test_get_available_empath_v15_models_filters_and_sorts(self):
        """It filters to empath-v15 directories and returns sorted model names."""
        host = _Host(project_root="K:/fake_root")
        with patch("os.path.exists", return_value=True), patch(
            "os.listdir",
            return_value=["empath-v15-zeta", "empath-v15-alpha", "not-a-model", "empath-v15-file"],
        ), patch(
            "os.path.isdir",
            side_effect=lambda p: p.endswith("empath-v15-zeta")
            or p.endswith("empath-v15-alpha")
            or p.endswith("not-a-model"),
        ):
            self.assertEqual(
                host.get_available_empath_v15_models(),
                ["empath-v15-alpha", "empath-v15-zeta"],
            )
    ...
    def test_initialize_service_uses_provided_project_root_for_checkpoints(self):
        ...
        self.assertTrue(ok)
        expected_model_path = os.path.normpath("K:/custom_root/checkpoints/empath-v15-turbo")
    ...
    def test_initialize_service_success_uses_decomposed_helpers(self):
        ...
        self.assertEqual(host.last_init_params["config_path"], "empath-v15-turbo")
...
ORCHESTRATOR_MODULE = importlib.import_module(
    "empath.core.generation.handler.init_service_orchestrator"
)
GPU_CONFIG_MODULE = importlib.import_module("empath.gpu_config")
MEMORY_UTILS_MODULE = importlib.import_module(
    "empath.core.generation.handler.memory_utils"
)
...
class RocmDtypeTests(unittest.TestCase):
    ...
    def test_resolve_rocm_dtype_defaults_to_float32(self):
        """It returns float32 when EMPATH_ROCM_DTYPE is not set."""
        with patch.dict("os.environ", {}, clear=False):
            os.environ.pop("EMPATH_ROCM_DTYPE", None)
    ...
    def test_resolve_rocm_dtype_respects_float16_override(self):
        """It returns float16 when EMPATH_ROCM_DTYPE=float16."""
        with patch.dict("os.environ", {"EMPATH_ROCM_DTYPE": "float16"}):
            result = ORCHESTRATOR_MODULE._resolve_rocm_dtype()
        self.assertEqual(result, torch.float16)

    def test_resolve_rocm_dtype_respects_bfloat16_override(self):
        """It returns bfloat16 when EMPATH_ROCM_DTYPE=bfloat16."""
        with patch.dict("os.environ", {"EMPATH_ROCM_DTYPE": "bfloat16"}):
            result = ORCHESTRATOR_MODULE._resolve_rocm_dtype()
        self.assertEqual(result, torch.bfloat16)

    def test_resolve_rocm_dtype_unknown_value_falls_back_to_float32(self):
        """It falls back to float32 for unrecognised EMPATH_ROCM_DTYPE values."""
        with patch.dict("os.environ", {"EMPATH_ROCM_DTYPE": "int8"}):
            result = ORCHESTRATOR_MODULE._resolve_rocm_dtype()
        self.assertEqual(result, torch.float32)
