...
from empath.gpu_config import get_effective_free_vram_gb
        ...
        with torch.inference_mode():
            with self._load_model_context("vae"):
                ...
                if not using_mlx_vae:
                    vae_cpu = os.environ.get("EMPATH_VAE_ON_CPU", "0").lower() in ("1", "true", "yes")
                    if not vae_cpu:
                        if self.device == "mps":
                            logger.info(
                            ...
                            if effective_free < 0.5:
                                ...
                                vae_cpu = True
                    if vae_cpu:
                        logger.info("[generate_music] Moving VAE to CPU for decode (EMPATH_VAE_ON_CPU=1)...")
