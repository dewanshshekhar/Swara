...
def _resolve_rocm_dtype() -> torch.dtype:
    ...
    AMD iGPUs on Strix Halo).  Set the ``EMPATH_ROCM_DTYPE`` environment
    variable to ``float16`` or ``bfloat16`` to override for hardware that
    fully supports those formats.
    """
    raw = os.environ.get("EMPATH_ROCM_DTYPE", "float32").strip().lower()
    dtype = _ROCM_DTYPE_MAP.get(raw)
    if dtype is None:
        logger.warning(
            f"[initialize_service] Unknown EMPATH_ROCM_DTYPE={raw!r}; "
    ...
    def initialize_service(
        ...
        try:
            if config_path is None:
                ...
                logger.warning(
                    "[initialize_service] config_path not set; defaulting to 'empath-v15-turbo'."
                )
            ...
            if resolved_device == "cuda" and gpu_config.is_rocm_available():
                ...
                logger.info(
                    ...
                    "(set EMPATH_ROCM_DTYPE=bfloat16 or float16 to override)"
                )
            else:
