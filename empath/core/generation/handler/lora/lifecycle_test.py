...
from empath.core.generation.handler.lora import lifecycle
...
class LifecycleTests(unittest.TestCase):
    ...
    def test_resolve_lokr_weights_from_custom_safetensors_name(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch(
                "empath.core.generation.handler.lora.lifecycle._is_lokr_safetensors",
    ...
    def test_load_lora_accepts_lokr_directory_without_adapter_config(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            weights.write_bytes(b"")
            with patch("empath.core.generation.handler.lora.lifecycle._load_lokr_adapter") as mock_load_lokr:
    ...
    def test_load_lokr_adapter_recreates_with_dora_when_weight_decompose_enabled(self):
        ...
        with patch.dict("sys.modules", {"lycoris": fake_lycoris}):
            with patch("empath.core.generation.handler.lora.lifecycle._load_lokr_config", return_value=config):
    ...
    def test_load_lokr_adapter_uses_base_net_when_dora_not_supported(self):
        ...
        with patch.dict("sys.modules", {"lycoris": fake_lycoris}):
            with patch("empath.core.generation.handler.lora.lifecycle._load_lokr_config", return_value=config):
                with patch("empath.core.generation.handler.lora.lifecycle.logger.warning") as mock_warning:
                    result = lifecycle._load_lokr_adapter(decoder, "weights.safetensors")
