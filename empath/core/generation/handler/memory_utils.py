...
from empath.gpu_config import get_effective_free_vram_gb, get_global_gpu_config, is_rocm_available
...
class MemoryUtilsMixin:
    ...
    def _get_auto_decode_chunk_size(self) -> int:
        ...
        override = os.environ.get("EMPATH_VAE_DECODE_CHUNK_SIZE")
        if override:
            try:
                value = int(override)
                if value > 0:
    ...
    def _should_offload_wav_to_cpu(self) -> bool:
        ...
        override = os.environ.get("EMPATH_MPS_DECODE_OFFLOAD")
        if override:
    ...
    def _get_vae_dtype(self, device: Optional[str] = None) -> torch.dtype:
        ...
        if target_device in ["cuda", "xpu"]:
            if target_device == "cuda" and is_rocm_available():
                ...
                # value (float32 by default, or EMPATH_ROCM_DTYPE override).
