...
class InitServiceCatalogMixin:
    ...
    def get_available_empath_v15_models(self) -> List[str]:
        """Scan and return all model directory names starting with ``empath-v15-``."""
        project_root = self._get_project_root()
        checkpoint_dir = os.path.join(project_root, "checkpoints")
        ...
        if os.path.exists(checkpoint_dir):
            for item in os.listdir(checkpoint_dir):
                ...
                if os.path.isdir(item_path) and item.startswith("empath-v15-"):
                    models.append(item)
