...
from empath.core.generation.handler.service_generate_request import ServiceGenerateRequestMixin
...
class ServiceGenerateRequestMixinTests(unittest.TestCase):
    ...
    def test_seed_list_shorter_than_batch_gets_padded(self):
        ...
        host = _Host(is_turbo=False)
        with patch("empath.core.generation.handler.service_generate_request.random.randint", return_value=99):
            out = host._normalize_service_generate_inputs(
                captions=["a", "b", "c"],
                lyrics=["l1"],
                keys=None,
