    ...
    with app_state._llm_init_lock:
        ...
        if getattr(app_state, "_llm_lazy_load_disabled", False):
            app_state._llm_init_error = (
                "LLM not initialized at startup. To enable LLM, set EMPATH_INIT_LLM=true "
        ...
        if init_llm_env in {"0", "false", "no", "n", "off"}:
            ...
            app_state._llm_init_error = (
                "LLM disabled via EMPATH_INIT_LLM=false. "
                "Optional LLM features (use_cot_caption, use_cot_language) will be auto-disabled."
            )
            log_fn("[API Server] LLM lazy load blocked: EMPATH_INIT_LLM=false")
            return

        project_root = get_project_root()
        checkpoint_dir = os.path.join(project_root, "checkpoints")
        lm_model_path = (
            req.lm_model_path or os.getenv("EMPATH_LM_MODEL_PATH") or "empath-5Hz-lm-0.6B"
        ).strip()
        backend = (req.lm_backend or os.getenv("EMPATH_LM_BACKEND") or "vllm").strip().lower()
        ...
        lm_device = os.getenv("EMPATH_LM_DEVICE", os.getenv("EMPATH_DEVICE", "auto"))
        lm_offload = env_bool("EMPATH_LM_OFFLOAD_TO_CPU", False)
