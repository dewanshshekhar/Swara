...
class JobBlockingGenerationTests(unittest.TestCase):
    ...
    def test_run_blocking_generate_success_path_updates_progress_and_builds_payload(self) -> None:
        ...
        with patch("empath.api.job_blocking_generation.prepare_llm_generation_inputs", return_value=prepared), \
                patch("empath.api.job_blocking_generation.build_generation_setup", return_value=setup), \
                patch("empath.api.job_blocking_generation.maybe_handle_analysis_only_modes", return_value=None), \
                patch(
                    "empath.api.job_blocking_generation.run_generation_with_optional_sequential_cover_mode",
                    side_effect=_run_generation_side_effect,
                ), \
                patch(
                    "empath.api.job_blocking_generation.build_generation_success_response",
                    return_value={"status_message": "Success"},
                ) as build_resp_mock, \
                patch("empath.api.job_blocking_generation.update_progress_job_cache") as cache_progress_mock:
    ...
    def test_run_blocking_generate_returns_analysis_result_without_generation(self) -> None:
        ...
        with patch("empath.api.job_blocking_generation.prepare_llm_generation_inputs", return_value=prepared), \
                patch("empath.api.job_blocking_generation.build_generation_setup", return_value=setup), \
                patch(
                    "empath.api.job_blocking_generation.maybe_handle_analysis_only_modes",
                    return_value=analysis_payload,
                ) as analysis_mock, \
                patch("empath.api.job_blocking_generation.run_generation_with_optional_sequential_cover_mode") as run_mock, \
                patch("empath.api.job_blocking_generation.build_generation_success_response") as build_resp_mock:
            result = run_blocking_generate(
                app_state=app_state,
                req=req,
                job_id="job-2",
