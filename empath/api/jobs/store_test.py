...
from empath.api.jobs.store import _JobStore, _append_jsonl, _atomic_write_json

_REPO_ROOT = Path(__file__).resolve().parents[3]
_REPO_TMP_ROOT = _REPO_ROOT / "empath" / "api" / "jobs"
...
class JobStorePersistenceTests(unittest.TestCase):
    ...
    def test_atomic_write_json_uses_atomic_replace_flow(self):
        ...
        with mock.patch("empath.api.jobs.store.tempfile.mkstemp", return_value=(fake_fd, fake_tmp_path)) as mkstemp_mock, \
             mock.patch("empath.api.jobs.store.os.fdopen", return_value=fake_file) as fdopen_mock, \
             mock.patch("empath.api.jobs.store.json.dump") as dump_mock, \
             mock.patch("empath.api.jobs.store.os.fsync") as fsync_mock, \
             mock.patch("empath.api.jobs.store.os.replace") as replace_mock:
    ...
    def test_append_jsonl_writes_one_json_line_per_record(self):
        ...
        with mock.patch("empath.api.jobs.store.open", return_value=fake_file, create=True) as open_mock:
