...
from empath.api.http.release_task_request_parser import parse_release_task_request
...
class ReleaseTaskRequestParserTests(unittest.TestCase):
    ...
    def test_multipart_cleanup_removes_uploaded_temp_files_on_error(self):
        ...
        with mock.patch("empath.api.http.release_task_request_parser.os.remove") as remove_mock:
            with self.assertRaises(HTTPException) as ctx:
                asyncio.run(
                    parse_release_task_request(
                        request=request,
