        ...
        with llm_lock:
            if not getattr(app.state, "_llm_initialized", False):
                ...
                if getattr(app.state, "_llm_lazy_load_disabled", False):
                    raise HTTPException(
                        status_code=503,
                        detail="LLM not initialized. Set EMPATH_INIT_LLM=true in .env to enable.",
                    )

                project_root = get_project_root()
                checkpoint_dir = os.path.join(project_root, "checkpoints")
                lm_model_path = os.getenv("EMPATH_LM_MODEL_PATH", "empath-5Hz-lm-0.6B").strip()
                backend = os.getenv("EMPATH_LM_BACKEND", "vllm").strip().lower()
