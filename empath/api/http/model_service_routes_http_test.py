...
class ModelServiceRoutesHttpTests(unittest.TestCase):
    ...
    def test_init_requires_authentication(self):
        ...
        client = self._build_client()
        response = client.post("/v1/init", json={"model": "empath-v15-base", "init_llm": False})
        self.assertEqual(401, response.status_code)
    ...
    def test_health_returns_wrapped_payload(self):
        ...
        self.assertEqual("Empath API", payload["data"]["service"])
    ...
    def test_init_route_returns_wrapped_success(self):
        ...
        client = self._build_client()
        with mock.patch(
            "empath.api.http.model_service_routes.initialize_models_for_request",
            return_value={"loaded_model": "empath-v15-base", "loaded_lm_model": None},
        ):
            response = client.post(
                "/v1/init",
                headers={"Authorization": "Bearer test-token"},
                json={"model": "empath-v15-base", "init_llm": False},
            )
    ...
    def test_init_route_returns_wrapped_error_when_llm_init_fails(self):
        ...
        with mock.patch("empath.api.http.model_init_service.os.makedirs"):
            response = client.post(
                "/v1/init",
                headers={"Authorization": "Bearer test-token"},
                json={"model": "empath-v15-base", "init_llm": True, "lm_model_path": "empath-5Hz-lm-0.6B"},
            )
