...
from empath.api.http.model_init_service import initialize_models_for_request


class InitModelRequest(BaseModel):
    """Request payload for on-demand DiT/LM model initialization."""

    model: Optional[str] = Field(default=None, description="DiT model name to initialize (e.g., 'empath-v15-base')")
    init_llm: bool = Field(default=False, description="Whether to initialize LLM as part of this request")
    lm_model_path: Optional[str] = Field(default=None, description="LLM model path/name (e.g., 'empath-5Hz-lm-1.7B')")
    ...
    if os.path.isdir(checkpoint_dir):
        for name in os.listdir(checkpoint_dir):
            ...
            if not os.path.isdir(full_path):
                continue
            if name.startswith("empath-5Hz-lm-"):
                available_lm_models.add(name)
            elif name.startswith("empath-"):
