...
from empath.gpu_config import VRAM_AUTO_OFFLOAD_THRESHOLD_GB, get_gpu_config
...
def initialize_models_for_request(
    ...
    offload_to_cpu_env = os.getenv("EMPATH_OFFLOAD_TO_CPU")
    offload_to_cpu = env_bool("EMPATH_OFFLOAD_TO_CPU", False) if offload_to_cpu_env is not None else auto_offload
    use_flash_attention = env_bool("EMPATH_USE_FLASH_ATTENTION", True)
    offload_dit_to_cpu = env_bool("EMPATH_OFFLOAD_DIT_TO_CPU", False)
    compile_model = env_bool("EMPATH_COMPILE_MODEL", False)
    ...
    if init_llm:
        lm_model_path = (requested_lm_model_path or os.getenv("EMPATH_LM_MODEL_PATH", "empath-5Hz-lm-0.6B")).strip()
        if not lm_model_path:
            lm_model_path = "empath-5Hz-lm-0.6B"

        os.environ["EMPATH_INIT_LLM"] = "true"
        os.environ["EMPATH_LM_MODEL_PATH"] = lm_model_path
