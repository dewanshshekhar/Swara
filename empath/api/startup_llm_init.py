...
from empath.gpu_config import get_recommended_lm_model, is_lm_model_supported
...
def initialize_llm_at_startup(
    *,
    ...
    init_llm_env = os.getenv("EMPATH_INIT_LLM", "").strip().lower()
    ...
    if not init_llm_env or init_llm_env == "auto":
        print("[API Server] EMPATH_INIT_LLM=auto, using GPU auto-detection result")
    elif init_llm_env in {"1", "true", "yes", "y", "on"}:
        if init_llm:
            print("[API Server] EMPATH_INIT_LLM=true (GPU already supports LLM, no override needed)")
        else:
            init_llm = True
            print("[API Server] EMPATH_INIT_LLM=true, overriding GPU auto-detection (force enable)")
    else:
        if not init_llm:
            print("[API Server] EMPATH_INIT_LLM=false (GPU already disabled LLM, no override needed)")
        else:
            init_llm = False
            print("[API Server] EMPATH_INIT_LLM=false, overriding GPU auto-detection (force disable)")

    if init_llm:
        print("[API Server] Loading LLM model...")
        lm_model_path = os.getenv("EMPATH_LM_MODEL_PATH", "").strip()
        if lm_model_path:
        ...
        lm_backend = os.getenv("EMPATH_LM_BACKEND", "vllm").strip().lower()
        if lm_backend not in {"vllm", "pt", "mlx"}:
            lm_backend = "vllm"
        lm_device = os.getenv("EMPATH_LM_DEVICE", device)
        lm_offload_env = os.getenv("EMPATH_LM_OFFLOAD_TO_CPU")
        lm_offload = env_bool("EMPATH_LM_OFFLOAD_TO_CPU", False) if lm_offload_env is not None else offload_to_cpu
    ...
    print("[API Server]   - Set EMPATH_INIT_LLM=true in .env or environment")
