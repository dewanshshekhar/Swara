...
class ServerCliTests(unittest.TestCase):
    ...
    def test_run_api_server_main_invokes_uvicorn_with_parsed_host_port(self) -> None:
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch("empath.api.server_cli.uvicorn.run") as mock_run:
    ...
    def test_run_api_server_main_sets_env_overrides_from_flags(self) -> None:
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch("empath.api.server_cli.uvicorn.run"):
                run_api_server_main(
            ...
            self.assertEqual("secret", os.environ.get("EMPATH_API_KEY"))
            self.assertEqual("huggingface", os.environ.get("EMPATH_DOWNLOAD_SOURCE"))
            self.assertEqual("true", os.environ.get("EMPATH_INIT_LLM"))
            self.assertEqual("empath-5Hz-lm-0.6B", os.environ.get("EMPATH_LM_MODEL_PATH"))
            self.assertEqual("true", os.environ.get("EMPATH_NO_INIT"))
    ...
    def test_run_api_server_main_uses_env_bool_for_flag_defaults(self) -> None:
        ...
        with patch.dict(os.environ, {}, clear=True):
            with patch("empath.api.server_cli.uvicorn.run") as mock_run:
                ...
                self.assertEqual("true", os.environ.get("EMPATH_INIT_LLM"))
                self.assertIsNone(os.environ.get("EMPATH_NO_INIT"))
                mock_run.assert_called_once()

        self.assertIn(("EMPATH_INIT_LLM", False), observed)
        self.assertIn(("EMPATH_NO_INIT", False), observed)
