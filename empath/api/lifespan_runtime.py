...
def _initialize_local_cache(app: Any, cache_root: str) -> None:
    ...
    try:
        from empath.local_cache import get_local_cache
...
def initialize_lifespan_runtime(
    *,
    ...
    cache_root = os.path.join(project_root, ".cache", "empath")
    tmp_root = (os.getenv("EMPATH_TMPDIR") or os.path.join(cache_root, "tmp")).strip()
    ...
    handler2 = None
    handler3 = None
    config_path2 = os.getenv("EMPATH_CONFIG_PATH2", "").strip()
    config_path3 = os.getenv("EMPATH_CONFIG_PATH3", "").strip()
    if config_path2:
    ...
    app.state._config_path = os.getenv("EMPATH_CONFIG_PATH", "empath-v15-turbo")
    app.state._config_path2 = config_path2
    app.state._config_path3 = config_path3

    max_workers = int(os.getenv("EMPATH_API_WORKERS", "1"))
