...
class JobRuntimeStateTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_cleanup_job_temp_files_removes_tracked_paths(self) -> None:
        ...
        fd, file_path = tempfile.mkstemp(prefix="empath-runtime-state-", suffix=".tmp")
        os.close(fd)
    ...
    @patch("empath.api.job_runtime_state.update_local_cache")
    @patch("empath.api.job_runtime_state.update_local_cache_progress")
