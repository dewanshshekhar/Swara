...
def iter_module_wrappers(module: nn.Module) -> list:
    ...
    Empath's ``trainer.py`` to ensure parity.
    """
    modules: list = []
    stack = [module]
    visited: set = set()
...
def configure_memory_features(decoder: nn.Module) -> tuple:
    ...
    Mirrors Empath's ``_configure_training_memory_features()`` exactly
    ...
    for mod in iter_module_wrappers(decoder):
        ...
        if hasattr(mod, "enable_input_require_grads"):
            try:
                ...
                hook_ok = bool(getattr(mod, "_empath_input_grads_hook_enabled", False))
