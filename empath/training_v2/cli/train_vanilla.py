...
``LoKRTrainer``) from ``empath/training/``.  Always prints a warning that
...
# Vanilla mode requires full Empath installation.
from empath.training.configs import LoRAConfig, TrainingConfig
from empath.training.trainer import LoRATrainer
_VANILLA_AVAILABLE = True

# LoKR may not exist in older Empath installs
try:
    from empath.training.configs import LoKRConfig
    from empath.training.trainer import LoKRTrainer
...
from empath.training_v2.gpu_utils import detect_gpu
from empath.training_v2.model_loader import load_decoder_for_training
...
def run_vanilla(args: argparse.Namespace) -> int:
    ...
    """
    # -- UI setup -------------------------------------------------------------
    from empath.training_v2.ui import set_plain_mode
    from empath.training_v2.ui.banner import show_banner
    from empath.training_v2.ui.errors import handle_error, show_info, show_warning
    from empath.training_v2.ui.progress import track_training
    from empath.training_v2.ui.summary import show_summary
    ...
    if not _VANILLA_AVAILABLE:
        show_warning(
            "Vanilla training requires a full Empath 1.5 installation.\n"
            "Install it with:  pip install -e /path/to/Empath-1.5\n"
    ...
    if args.model_variant in ("base", "sft"):
        from empath.training_v2.ui.config_panel import confirm_start
        from empath.training_v2.ui.errors import show_error
    ...
    if adapter_type == "lokr" and not _LOKR_AVAILABLE:
        ...
        show_error(
            ...
            message=(
                "Your Empath installation does not include LoKR support.\n"
                "Please update Empath to a version with lokr_utils, or use LoRA instead."
            ),
        )
        return 1
