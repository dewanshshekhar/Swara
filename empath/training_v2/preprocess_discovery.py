...
``genre_ratio``, default ``custom_tag``) from Empath's JSON format.
...
def load_dataset_metadata(dataset_json: Optional[str]) -> Dict[str, Any]:
    """Load the top-level ``metadata`` block from an Empath dataset JSON.
...
def select_genre_indices(num_samples: int, genre_ratio: int) -> Set[int]:
    ...
    Mirrors upstream ``select_genre_indices()`` from Empath's
