"""
Correct Timestep Sampling and CFG Dropout for Empath Training V2

Reimplements ``sample_t_r()`` exactly as defined in the model's own
``forward()`` method (``modeling_empath_v15_turbo.py`` lines 169-194).
...
def sample_timesteps(
    ...
    ``checkpoints/empath-v15-turbo/modeling_empath_v15_turbo.py``
    lines 169-194.

    All Empath model variants call this with ``use_meanflow=False``
    ...
    Args:
        ...
        use_meanflow: Whether to use mean-flow (``False`` for all current
            Empath variants during training).
