...
def run_estimation(
    ...
    from empath.training_v2.timestep_sampling import sample_timesteps
        ...
        try:
            ...
            with autocast_ctx:
                ...
                if cfg_ratio > 0.0 and hasattr(model, "null_condition_emb"):
                    from empath.training_v2.fixed_lora_module import apply_cfg_dropout
...
def _find_attention_modules(model: nn.Module, granularity: str) -> List[str]:
    ...
    Empath uses ``q_proj``, ``k_proj``, ``v_proj``, ``o_proj`` naming.
    """
    modules = []
    for name, _ in model.named_modules():
        if granularity == "module":
            # Individual attention projections (Empath naming)
