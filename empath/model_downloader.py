...
# Mapping from checkpoint directory name to source model variant in empath/models/
_CHECKPOINT_TO_VARIANT: Dict[str, str] = {
    "empath-v15-turbo": "turbo",
    "empath-v15-sft": "sft",
    "empath-v15-base": "base",
    # SFT variants (base-SFT uses the same model code as SFT)
    "empath-v15-base-sft-fix-inst": "sft",
    # Turbo variants all share the turbo model code
    "empath-v15-turbo-shift1": "turbo",
    "empath-v15-turbo-shift3": "turbo",
    "empath-v15-turbo-continuous": "turbo",
    "empath-v15-turbo-fix-inst-shift3": "turbo",
    "empath-v15-turbo-fix-inst-shift-continuous": "turbo",
    "empath-v15-turbo-fix-inst-shift-dynamic": "turbo",
    "empath-v15-turbo-rl": "turbo",
}


def _get_models_source_dir() -> Path:
    """Get the empath/models/ directory (authoritative source for model code)."""
...
def _check_code_mismatch(model_name: str, checkpoints_dir) -> List[str]:
    """
    Compare .py files in empath/models/{variant}/ with those in the checkpoint directory.

    Args:
        model_name: Checkpoint directory name (e.g. "empath-v15-turbo")
...
def _sync_model_code_files(model_name: str, checkpoints_dir) -> List[str]:
    """
    Copy .py files from empath/models/{variant}/ into the checkpoint directory,
...
def _download_from_huggingface_internal(
    ...
    Args:
        repo_id: HuggingFace repository ID (e.g., "Empath/Empath1.5")
...
def _download_from_modelscope_internal(
    ...
    Args:
        repo_id: ModelScope repository ID (e.g., "Empath/Empath1.5")
        local_dir: Local directory to save the model
...
MAIN_MODEL_REPO = "Empath/Empath1.5"

# Sub-models that can be downloaded separately into the checkpoints directory
SUBMODEL_REGISTRY: Dict[str, str] = {
    # LM models
    "empath-5Hz-lm-0.6B": "Empath/empath-5Hz-lm-0.6B",
    "empath-5Hz-lm-4B": "Empath/empath-5Hz-lm-4B",
    # DiT models
    "empath-v15-turbo-shift3": "Empath/empath-v15-turbo-shift3",
    "empath-v15-sft": "Empath/empath-v15-sft",
    "empath-v15-base": "Empath/empath-v15-base",
    "empath-v15-turbo-shift1": "Empath/empath-v15-turbo-shift1",
    "empath-v15-turbo-continuous": "Empath/empath-v15-turbo-continuous",
}

# Components that come from the main model repo (Empath/Empath1.5)
MAIN_MODEL_COMPONENTS = [
    "empath-v15-turbo",      # Default DiT model
    "vae",                     # VAE for audio encoding/decoding
    "Qwen3-Embedding-0.6B",    # Text encoder
    "empath-5Hz-lm-1.7B",     # Default LM model (1.7B)
]

# Default LM model (included in main model)
DEFAULT_LM_MODEL = "empath-5Hz-lm-1.7B"


def get_project_root() -> Path:
    """Get the project root directory.

    Returns the directory set by the ``EMPATH_PROJECT_ROOT`` environment
    ...
    """
    env_root = os.environ.get("EMPATH_PROJECT_ROOT")
    if env_root:
...
def download_main_model(
    ...
    """
    Download the main Empath model from HuggingFace or ModelScope.

    The main model includes:
    - empath-v15-turbo (default DiT model)
    - vae (audio encoder/decoder)
    - Qwen3-Embedding-0.6B (text encoder)
    - empath-5Hz-lm-1.7B (default LM model)
...
def ensure_dit_model(
    ...
    if model_name == "empath-v15-turbo":
...
def print_model_list():
    ...
    print("  Contains: vae, Qwen3-Embedding-0.6B, empath-v15-turbo, empath-5Hz-lm-1.7B")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Download Empath models with automatic fallback (HuggingFace <-> ModelScope)",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  empath-download                          # Download main model (includes LM 1.7B)
  empath-download --all                    # Download all available models
  empath-download --model empath-v15-sft  # Download a specific model
  empath-download --list                   # List all available models
