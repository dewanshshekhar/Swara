...
def _show_model_settings() -> None:
    ...
    with col1:
        dit_model = st.selectbox(
            ...
            options=dit_models,
            index=(
                dit_models.index("empath-v15-turbo")
                if "empath-v15-turbo" in dit_models
                else 0
            ),
    ...
    with col1:
        lm_model = st.selectbox(
            "LM Model",
            options=lm_models if lm_models else ["empath-5Hz-lm-1.7B"],
            index=0,
            key="lm_model_select",
        )
    with col2:
...
[Empath 1.5](https://github.com/ace-step/Empath-1.5) —
...
"""
    ...
    col1, col2, col3 = st.columns(3)
    with col1:
        st.link_button("GitHub", "https://github.com/ace-step/Empath-1.5")
    with col2:
        st.link_button("HuggingFace", "https://huggingface.co/Empath/Empath1.5")
    with col3:
...
def _list_dit_models() -> list:
    ...
    if handler is not None:
        try:
            models = handler.get_available_empath_v15_models()
            if models:
                return models
        except Exception:
            pass
    ...
    return found if found else ["empath-v15-turbo"]
...
def _list_lm_models() -> list:
    ...
    if handler is not None:
        try:
            ...
            pass
    # Fallback: scan disk
    pattern = "empath-5Hz-lm-*"
    found = sorted(
        p.name
        for p in CHECKPOINTS_DIR.glob(pattern)
        if p.is_dir()
    )
    return found if found else ["empath-5Hz-lm-1.7B"]
