...
Centralized GPU memory detection and adaptive configuration management
    ...
    Debug Mode:
        ...
        Example: MAX_CUDA_VRAM=8 python empath  # Simulates 8GB GPU

        For MPS testing, use MAX_MPS_VRAM to simulate MPS memory.
        Example: MAX_MPS_VRAM=16 python empath  # Simulates 16GB MPS
...
GPU_TIER_CONFIGS = {
    ...
    "tier3": {  # 6-8GB
        ...
        "available_lm_models": ["empath-5Hz-lm-0.6B"],
        "recommended_lm_model": "empath-5Hz-lm-0.6B",
        "lm_backend_restriction": "all",
        "recommended_backend": "vllm",
        "offload_to_cpu_default": True,
        "offload_dit_to_cpu_default": True,
    ...
    "tier5": {  # 12-16GB
        ...
        "max_batch_size_without_lm": 4,
        "init_lm_default": True,
        "available_lm_models": ["empath-5Hz-lm-0.6B", "empath-5Hz-lm-1.7B"],
        "recommended_lm_model": "empath-5Hz-lm-1.7B",
        "lm_backend_restriction": "all",
        "recommended_backend": "vllm",
        "offload_to_cpu_default": True,
    ...
    "unlimited": {  # >= 24GB
        ...
        "recommended_lm_model": "empath-5Hz-lm-4B",
        "lm_backend_restriction": "all",
        "recommended_backend": "vllm",
...
def _log_gpu_diagnostic_info(torch_module):
    ...
    if is_rocm_build:
        ...
        logger.warning(
            "  5. See docs/en/Empath1.5-Rocm-Manual-Linux.md for Linux setup"
        )
        logger.warning(
...
def get_lm_model_size(model_path: str) -> str:
    ...
    Args:
        model_path: Model path string (e.g., "empath-5Hz-lm-0.6B", "empath-5Hz-lm-0.6B-v4-fix")
...
def is_lm_model_size_allowed(
    ...
    Uses size-based matching so that variants like "empath-5Hz-lm-0.6B-v4-fix"
    are correctly matched against "empath-5Hz-lm-0.6B" in the tier config.

    Args:
        disk_model_name: Actual model directory name on disk (e.g., "empath-5Hz-lm-0.6B-v4-fix")
        tier_available_models: List of tier-allowed model base names (e.g., ["empath-5Hz-lm-0.6B"])
...
def find_best_lm_model_on_disk(
    ...
    Args:
        recommended_model: Tier-recommended model name (e.g., "empath-5Hz-lm-0.6B")
...
def get_lm_gpu_memory_ratio(
    ...
    Args:
        model_path: LM model path (e.g., "empath-5Hz-lm-0.6B")
...
def compute_adaptive_config(total_vram_gb: float, dit_type: str = "turbo") -> GPUConfig:
    ...
    for size_key in ["0.6B", "1.7B", "4B"]:
        ...
        if lm_total + inference_per_batch <= available:
            model_name = f"empath-5Hz-lm-{size_key}"
