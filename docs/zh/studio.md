...
Empath includes an optional, experimental HTML-based Studio UI for users who want a more structured, DAW-like interface.
...
1. Start the Empath API server (e.g. `uv run empath --enable-api --port 8001` or your usual API launch command).
2. Open `ui/studio.html` in a browser (double-click or `file:///path/to/Empath-1.5/ui/studio.html`).
...
- **Optional:** The default way to use Empath remains the Gradio Web UI.
