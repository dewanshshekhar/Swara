#!/usr/bin/env bash
# Empath Gradio Web UI Launcher - macOS (Apple Silicon / MLX)
...
echo "============================================"
echo "  Empath 1.5 - macOS Apple Silicon (MLX)"
echo "============================================"
echo
...
if [[ -f "$SCRIPT_DIR/python_embedded/bin/python3.11" ]]; then
    ...
    if "$SCRIPT_DIR/python_embedded/bin/python3.11" -c "pass" 2>/dev/null; then
        ...
        SCRIPT_PATH="$SCRIPT_DIR/empath/empath_v15_pipeline.py"
...
echo "Starting Empath Gradio UI (MLX backend)..."
echo
...
[[ -n "$BACKEND" ]] && EMPATH_ARGS="$EMPATH_ARGS $BACKEND"
